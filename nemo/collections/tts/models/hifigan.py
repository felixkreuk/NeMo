...
class HifiGanModel(Vocoder):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if batch_idx == 0 and isinstance(self.logger, WandbLogger):
            for i in range(min(3, audio.shape[0])):
                self.logger.experiment.log({
                    "audio": [
                        wandb.Audio(
                            audio[i].data.cpu().numpy(),
                            caption=f"real audio {i}",
                            sample_rate=self.sample_rate
                        ),
                        wandb.Audio(
                            audio_pred[i, 0].data.cpu().numpy(),
                    ...
                    "specs": [
                        wandb.Image(
                            plot_spectrogram_to_numpy(audio_mel[i].data.cpu().numpy()),
                            caption=f"real audio {i}"
                        ),
                        wandb.Image(
                            plot_spectrogram_to_numpy(audio_pred_mel[i].data.cpu().numpy()),
                            caption=f"generated audio {i}"
                        )
                    ],
                },
